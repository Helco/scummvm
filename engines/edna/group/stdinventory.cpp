...
void StdInventory::updateSelection(Sprite *selection) {
	...
	} else if (selection != nullptr && &selection->group() == this && selection->id() > 0)
...
void StdInventory::updateItems() {
	...
	constexpr uint kSlotsPerLine = 5;
	constexpr int16 kLineCount = 7;

	// both these spans are sorted by position so we do not reload sprites we already have
	// selecting for StartMenu instead of EdnaStd is an original quirk
	auto dbItems = g_engine->db().ownedItems(GameMode::StartMenu);
	uint spriteI = kFirstItemI;

	uint maxScroll = (dbItems.size() + kSlotsPerLine - 1) / kSlotsPerLine;
	maxScroll = maxScroll < kLineCount ? 0 : maxScroll - kLineCount;
	...
	for (uint slotI = 0; slotI < dbItems.size(); slotI++, spriteI++) {
		...
		assert(item != nullptr);
		int16 line = (int16)(slotI / kSlotsPerLine) - _scroll;
		item->pos() = kOrigin + Point((slotI % kSlotsPerLine) * kSlotWidth, line * kSlotHeight);
		item->toggle(line >= 0 && line < kLineCount);
	}
